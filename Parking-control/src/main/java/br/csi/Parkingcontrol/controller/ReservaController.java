...
System.out.println(reservaModel);
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<String> deleteReserva(@PathVariable(value = "id")UUID id){
    ...
    @PatchMapping("{id}")
public  ResponseEntity<Object> updateReserva(@PathVariable(value = "id") UUID id, @RequestBody ReservaModel reservaModel){
        // Optional<ReservaModel> parkingSpotModelOptional = reservaService.findById(id);
        // if(!parkingSpotModelOptional.isPresent()){
        //     return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Reserva não encontrada");
        // }
        // var reservaModel = new ReservaModel();// seta só os dois que são fixos data e id
        // //BeanUtils.copyProperties(parkingSpotDto,vagaModel);
        // reservaModel.setId_reserva(parkingSpotModelOptional.get().getId_reserva());
